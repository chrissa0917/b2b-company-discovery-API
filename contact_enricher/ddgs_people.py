...
from .person_first_fast import candidate_emails, email_evidence
...
def _company_match(company: str, website: str, text: str) -> bool:
    ...
    tokens = _company_tokens(company, website)
    return any(re.sub(r"[^a-z0-9]", "", token) in haystack for token in tokens)
...
def _extract_name_title(title: str, body: str, requested: list[str]) -> tuple[str, str]:
    ...
    # Common indexed profile title: "Jane Doe - VP Marketing | LinkedIn"
    for sep in [" - ", " – ", " — ", " | ", " · "]:
        if sep in title:
    ...
    # Sometimes the result title is only the person's name; the role is in the body.
    ...
    # Final safe form: "Jane Doe, Chief Marketing Officer"
...
async def find_people_ddgs(company: str, website: str, requested: list[str]) -> list[ContactCandidate]:
    ...
    # DDGS is the open-source multi-search transport. Avoid Google as the preferred
    # path because Railway was rate-limited in the CrossLinked benchmark.
    backends = ["brave,duckduckgo,mojeek,startpage,yahoo", "auto"]
    tasks = [asyncio.create_task(_search(query, backend)) for query, backend in zip(queries, backends)]
    ...
    for group in groups:
        for row in group:
            ...
            if "linkedin.com/in" not in href.lower() and "linkedin.com/in" not in combined.lower():
                # Non-LinkedIn result can still be evidence, but it is too weak to
                # become the selected person in targeted mode.
                continue
...
async def enrich_person_ddgs(company: str, website: str, requested: list[str]) -> dict:
    people_task = asyncio.create_task(find_people_ddgs(company, website, requested))
    ...
    domain = domain_from_url(website)
    ranked = candidate_emails(person, public_emails, people, approved, domain) if person else []
    ...
    return {
        ...
        "Review Candidate Email": ranked[0]["email"] if ranked else "",
        "Email Confidence": f"{ranked[0]['score']}/100 — {ranked[0]['evidence']}" if ranked else "",
        "Email Pattern": ranked[0]["pattern"] if ranked else "",
        "Approved Email Domains": "; ".join(sorted(approved)),
        "Public Email Evidence": "; ".join(item.email for item in public_emails[:10]),
        "Candidate Emails": "; ".join(item["email"] for item in ranked[:6]),
